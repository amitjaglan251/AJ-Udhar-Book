...
class FirestoreSyncManager(
    ...
    suspend fun syncCustomer(customer: Customer) {
        ...
        val data = hashMapOf<String, Any>("id" to customer.id, "name" to customer.name, "mobile" to customer.mobile, "address" to customer.address, "createdAt" to customer.createdAt)
        requireUserDocument().collection("customers").document(customer.id.toString()).set(data).await()
    }
    ...
    suspend fun syncTransaction(transaction: Transaction) {
        ...
        val data = hashMapOf<String, Any>("id" to transaction.id, "customerId" to transaction.customerId, "amount" to transaction.amount, "type" to transaction.type, "note" to transaction.note, "timestamp" to transaction.timestamp)
        requireUserDocument().collection("transactions").document(transaction.id.toString()).set(data).await()
    }
    ...
    suspend fun restoreCloudToLocal() {
        ...
        val cloudCustomers = customerSnapshot.documents.mapNotNull { document ->
            ...
            Customer(id = id, name = document.getString("name") ?: "", mobile = document.getString("mobile") ?: "", address = document.getString("address") ?: "", createdAt = document.getLong("createdAt") ?: System.currentTimeMillis())
        }
        ...
        val cloudTransactions = transactionSnapshot.documents.mapNotNull { document ->
            ...
            Transaction(id = id, customerId = customerId, amount = document.getDouble("amount") ?: document.getLong("amount")?.toDouble() ?: 0.0, type = document.getString("type") ?: "UDHAR", note = document.getString("note") ?: "", timestamp = document.getLong("timestamp") ?: System.currentTimeMillis())
        }
        ...
        if (localTransactions.isEmpty() && cloudTransactions.isNotEmpty()) {
            val availableCustomerIds = customerDao.getAllCustomersOnce().asSequence().map { it.id }.toSet()
            ...
            for (transaction in validTransactions) {
                try {
                    ...
                    Log.d("AJ_RESTORE", "Transaction restored: id=${transaction.id}, customerId=${transaction.customerId}")
                } catch (e: Exception) {
